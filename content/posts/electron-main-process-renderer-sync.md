...
- [7. 임시 상태와 미디어 파일](#7-임시-상태와-미디어-파일)
...
따라서 `useSyncExternalStore`만 사용하려면 별도의 변수가 필요하다.
...
`currentSnapshot`이 실제 저장 공간이고 Hook은 변경 알림을 React render로 연결한다.
...
focus 때마다 디스크를 다시 읽거나 Main snapshot을 불필요하게 가져오지 않도록 기본값을 조정한다.

```ts
// project-query-options.ts
export const projectQueryOptions = {
  queryKey: ['project', 'active'] as const,
...
TanStack Query의 기본값은 stale query의 자동 refetch와 실패 query의 retry를 포함한다. local IPC cache에서는 의도와 다를 수 있으므로 명시적으로 정한다. [TanStack Query Important Defaults](https://tanstack.com/query/latest/docs/framework/react/guides/important-defaults)
...
await window.projectAPI.dispatch(action);
```
...
Renderer별 route guard만으로는 다른 창의 현재 mode를 알 수 없다. Main `WorkspaceModeStore`가 열린 mode를 확인하고 진입 요청을 허용하거나 거절한다. 이 Store는 프로젝트 문서가 아니라 창 접근 규칙을 관리한다.
...
Electron IPC는 Structured Clone을 사용하지만 DOM 객체 전송을 지원하지 않는다. `File` 자체를 Main으로 보내는 대신 local path나 byte chunk를 보낸다. [Electron ipcRenderer](https://www.electronjs.org/docs/latest/api/ipc-renderer)

- native file dialog: Main이 반환한 local path 사용
- drag and drop: Preload에서 `webUtils.getPathForFile(file)` 사용
- JavaScript에서 만든 memory `Blob`: 크기를 제한한 chunk 전송

Electron의 `webUtils.getPathForFile`은 Renderer의 `File` 객체에 대한 실제 파일 시스템 경로를 반환한다. [Electron webUtils](https://www.electronjs.org/docs/latest/api/web-utils)
...
interface ProjectUpdateResult {
  version: number;
  changedRow: { id: string; text: string };
}

interface ApplyUpdateInput {
  current: ProjectSnapshot;
  update: ProjectUpdateResult;
}

type ApplyUpdateResult =
  | { type: 'ignored'; snapshot: ProjectSnapshot }
  | { type: 'applied'; snapshot: ProjectSnapshot }
  | { type: 'snapshotRequired' };

export function applyProjectUpdate({ current, update }: ApplyUpdateInput): ApplyUpdateResult {
  if (update.version <= current.version) {
    return { type: 'ignored', snapshot: current };
  }

  if (update.version !== current.version + 1) {
    return { type: 'snapshotRequired' };
  }
  ...
  return {
    ...
    snapshot: {
      ...
      rows: {
        ...current.rows,
        [update.changedRow.id]: update.changedRow,
      },
    },
  };
}
