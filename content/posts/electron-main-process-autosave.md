...
description: 'Renderer의 저장 UI와 Main의 자동저장 책임을 나누고 복구의 보장 수준을 구분합니다.'
date: '2026-07-14'
...
- [2. 저장 책임 분리](#2-저장-책임-분리)
  - [2-1. Renderer의 저장 UI](#2-1-renderer의-저장-ui)
  - [2-2. Main의 저장 조정](#2-2-main의-저장-조정)
  - [2-3. 파일 쓰기 계층](#2-3-파일-쓰기-계층)
- [3. 기본 자동저장 흐름](#3-기본-자동저장-흐름)
  - [3-1. 한 번에 하나만 쓰기](#3-1-한-번에-하나만-쓰기)
  - [3-2. 최신 snapshot만 유지](#3-2-최신-snapshot만-유지)
  - [3-3. 저장 완료 version](#3-3-저장-완료-version)
- [4. 실시간 저장의 두 의미](#4-실시간-저장의-두-의미)
  - [4-1. 기본 선택](#4-1-기본-선택)
  - [4-2. 더 강한 보장이 필요할 때](#4-2-더-강한-보장이-필요할-때)
- [5. 안전한 파일 교체](#5-안전한-파일-교체)
  - [5-1. 새 프로젝트](#5-1-새-프로젝트)
  - [5-2. 종료 전 flush](#5-2-종료-전-flush)
- [6. 충돌과 오래된 비동기 결과](#6-충돌과-오래된-비동기-결과)
  - [6-1. 같은 row 수정](#6-1-같은-row-수정)
  - [6-2. 늦게 도착한 작업](#6-2-늦게-도착한-작업)
- [7. 장애 복구](#7-장애-복구)
- [8. 성능 확인](#8-성능-확인)
- [9. 마치며](#9-마치며)
...
결론부터 적으면, **자동저장 시점과 파일 쓰기는 Main이 담당하고 Renderer는 저장 버튼과 저장 상태 표시만 담당한다**. 기본안은 Main memory에 즉시 반영하고 디스크에는 debounce 후 저장하는 방식이다.
...
실제 파일 API 호출은 저장 시점과 version을 관리하는 책임에서 분리한다.
...
이 경계를 두면 자동저장 규칙과 운영체제 파일 작업을 따로 검증할 수 있다.
...
sequenceDiagram
  ...
  participant M as "Main 저장 조정"
  participant F as "파일 쓰기 계층"
...
Main memory의 현재 version과 project file 교체가 끝난 version을 따로 관리한다.
...
충돌을 확인하려면 변경 요청에 사용자가 편집을 시작한 시점의 item version을 함께 보낸다. 현재 version과 다르면 Main은 요청을 거절하고 최신 row를 돌려줄 수 있다. 자동 merge UI까지 제공할지는 별도 제품 결정이다.

### 6-2. 늦게 도착한 작업

TTS 생성이나 waveform 분석처럼 오래 걸리는 작업은 시작한 프로젝트가 이미 닫힌 뒤 끝날 수 있다. 작업을 시작한 프로젝트, 대상 item, 당시 version을 결과와 함께 확인한다. Main은 결과를 적용하기 직전에 현재 값과 비교하고 일치하지 않으면 오래된 결과로 판단한다. 취소 요청만으로 이 검증을 대체할 수는 없다.
...
이번 설계에서 가장 중요한 태도는 "실시간"이라는 표현을 구현 방식으로 바로 번역하지 않는 것이었다. 먼저 어느 실패까지 막아야 하는지 정해야 한다. 다음 글에서는 이 구조가 실제로 해결해야 할 문제와 선택의 비용을 다시 검증한다.

[다음 글: Part 6. Main SSOT 설계를 검증하고 선택의 비용 정리하기](/posts/electron-main-process-migration)
