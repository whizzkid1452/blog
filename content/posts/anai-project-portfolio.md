...
#### 676개 오디오 반영과 1,518개 Region 편집 성능 개선

오디오 반영 단계에서는 한꺼번에 실행되는 작업과 메모리 복사를 제한했습니다. 타임라인 편집 단계에서는 Region 조회와 React 렌더링 경로를 분리했습니다.

**관찰한 문제**

한 번의 음성 생성으로 676개 오디오가 만들어졌습니다. 기존에는 오디오를 하나 추가할 때마다 커지는 전체 배치 목록을 IPC로 다시 전송하고, 676개의 가져오기·디코딩·파형 생성 작업을 동시 실행했습니다. 실제 프로젝트에서 반영 시간 15.58초, Long Task 누적 2.24초, 내부 반영 작업 오류 78건, 최대 메모리 사용량 2.32GB가 측정됐습니다.

반영이 끝난 뒤에는 겹침 처리 결과를 포함해 1,518개 Region이 남았습니다. 사용자는 다중 트랙에서 Region이 500개를 넘으면 스크롤과 편집 입력이 늦어 작업하기 어렵다고 제보했습니다. 코드에서는 가로 스크롤마다 각 트랙의 Region 전체를 조회하고, 스크롤 프레임 값의 변경으로 모든 TrackRow가 다시 렌더링되는 경로를 확인했습니다.
...
1. **Bulk IPC:** 676개 결과를 배열로 모아 상태 변경과 IPC 호출을 `676회 → 1회`로 줄였습니다. IPC로 전달한 누적 레코드 수도 `228,826개 → 676개`로 감소했습니다.
2. **제한된 병행 실행:** 가져오기·디코딩·파형 생성 작업을 FIFO 작업 큐에 넣고 최대 4개만 동시에 실행했습니다. 한 개씩 순차 실행하지 않고, CPU와 메모리가 동시에 경쟁하는 작업 수만 제한했습니다.
3. **오래된 결과 차단:** 프로젝트 전환이나 Region 삭제 전에 시작된 작업은 완료되더라도 현재 프로젝트 상태를 변경하지 못하도록 버전을 검사했습니다.
4. **추가 복사 제거:** IPC로 받은 바이트 배열이 전체 버퍼를 가리킬 때는 해당 버퍼를 디코딩 입력으로 재사용했습니다. Electron IPC 경계의 복사가 아니라 Renderer에서 발생하던 추가 전체 복사 한 번을 제거한 것입니다.

**2. 타임라인 편집 경로**

1. **Region 시간 인덱스:** Region 목록이 바뀔 때 시작 시간순으로 정렬하고, 각 위치에 앞 구간까지의 최대 종료 시간을 저장했습니다. 스크롤 중에는 같은 인덱스를 재사용하고 두 번의 이진 탐색으로 현재 화면의 후보 구간만 찾습니다. 이를 통해 화면보다 먼저 시작해 현재 화면까지 이어지는 긴 Region도 누락하지 않으면서 전체 순회를 피했습니다.
2. **스크롤 렌더링 분리:** 기존에는 스크롤 프레임마다 바뀌는 prop 때문에 `React.memo`가 TrackRow 렌더링을 건너뛸 수 없었습니다. 이를 참조가 유지되는 스크롤 신호로 교체해 TrackRow의 React 렌더링은 건너뛰고 Canvas 갱신만 요청했습니다.

**선택 근거와 한계**

세로 Track 가상화는 적용하지 않았습니다. 실제 프로젝트가 주로 10~15개 Track을 사용해 화면 밖 Track을 제거하는 효과가 작았기 때문입니다. 반면 적용하려면 드래그, 선택, Track 재정렬과 서로 다른 Track 높이의 좌표계를 함께 바꿔야 했습니다. 현재 조건에서는 Region 수에 따라 증가하던 전체 조회와 스크롤 중 TrackRow 재렌더링을 먼저 줄이는 편이 변경 범위가 작았습니다.

모든 TrackRow는 계속 마운트되며 모든 Track Canvas가 스크롤 갱신 신호를 받습니다. 따라서 이번 변경은 세로 Track 가상화가 아니라, Region 가시 범위 조회와 TrackRow React 렌더링을 최적화한 작업입니다.
...
[상세 구현: 676개 오디오 반영부터 1,518개 Region 편집까지](/posts/large-scale-audio-processing-pipeline)
