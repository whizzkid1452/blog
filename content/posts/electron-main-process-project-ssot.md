...
- [2. SSOT의 범위](#2-ssot의-범위)
  - [2-1. 이 글의 정의](#2-1-이-글의-정의)
  - [2-2. Main에 두는 상태](#2-2-main에-두는-상태)
  - [2-3. Main에 두지 않는 상태](#2-3-main에-두지-않는-상태)
- [3. SSOT 위치 비교](#3-ssot-위치-비교)
- [4. Class private field와 Vanilla Zustand 비교](#4-class-private-field와-vanilla-zustand-비교)
  - [4-1. 처음 고려한 구조](#4-1-처음-고려한-구조)
  - [4-2. private field 선택](#4-2-private-field-선택)
  - [4-3. 다시 검토할 조건](#4-3-다시-검토할-조건)
- [5. 상태 분류](#5-상태-분류)
- [6. Main과 Renderer의 책임](#6-main과-renderer의-책임)
  - [6-1. Main](#6-1-main)
  - [6-2. Renderer](#6-2-renderer)
  - [6-3. ProjectSession이 하지 않는 일](#6-3-projectsession이-하지-않는-일)
- [7. 전체 구조](#7-전체-구조)
- [8. 선택의 조건](#8-선택의-조건)
- [9. 마치며](#9-마치며)
...
- 사용자 변경 요청을 검증하고 프로젝트 문서에 반영한다.
- version과 Undo/Redo History를 갱신한다.
- 확정된 변경 결과를 열린 Renderer에 발행한다.
- 최신 snapshot의 자동저장을 요청한다.

### 6-2. Renderer

- TanStack Query Cache에서 읽기 전용 `ProjectSnapshot`을 읽는다.
- Main의 확정 event로 cache를 갱신한다.
- document 변경 결과를 AudioEngine 같은 실행 객체에 반영한다.
- Zustand에는 selection과 modal과 drag preview 같은 UI state만 둔다.
- 저장 버튼과 파일 선택처럼 사용자와 직접 상호작용하는 UI를 담당한다.

### 6-3. ProjectSession이 하지 않는 일

`ProjectSession`이 Main의 모든 일을 맡으면 다시 큰 Class가 된다. 다음 책임은 별도 경계로 분리한다.
...
```mermaid
flowchart TB
  Editor["Editor Renderer\n읽기 전용 cache와 UI state"]
  Script["SRT Panel Renderer\n읽기 전용 cache와 UI state"]
  Admin["Admin Renderer\n읽기 전용 cache"]

  IPC["IPC 요청과 확정 event"]
