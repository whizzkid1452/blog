...
description: 'Next.js 서버와 Supabase를 연결해 댓글 저장소, API Route, RLS 정책과 댓글 UI를 구현한 과정을 정리합니다.'
date: '2026-07-14'
tags: ['nextjs', 'supabase', 'postgresql', 'security']
draft: true
---

정적 Markdown 글만 제공하던 블로그에 댓글 기능을 추가했다. 글은 파일로 관리할 수 있지만, 방문자가 작성한 댓글은 요청이 끝난 뒤에도 남아 있어야 한다. 그래서 Supabase의 PostgreSQL을 영구 저장소로 사용했다.
...
Repository는 데이터 저장소에 접근하는 인터페이스다. API Route는 Supabase 쿼리 작성법을 알 필요가 없고, 테스트에서는 실제 데이터베이스 대신 Stub을 주입할 수 있다.
...
길이 제한은 UI와 API에만 두지 않고 데이터베이스에도 뒀다. 다른 경로에서 테이블에 접근하더라도 빈 문자열이나 제한을 넘는 값이 저장되지 않게 하기 위해서다.
...
`Origin` 비교는 브라우저의 다른 출처에서 전송되는 쓰기 요청을 거부한다. 하지만 사용자 인증, 요청 횟수 제한, 스팸 차단을 제공하지는 않는다. `Origin`이 없는 비브라우저 요청을 막는 장치로 볼 수도 없다.

저장소 오류의 상세 메시지는 응답에 포함하지 않았다. 서버 로그에는 원래 오류를 남기고 사용자에게는 일반화된 `503` 메시지를 반환한다.
...
서버 응답도 Zod schema로 다시 확인한다. 응답 구조가 예상과 다르면 성공 화면을 표시하지 않는다. 등록에 성공하면 반환된 댓글을 현재 목록 뒤에 추가하고 댓글 본문만 비운다.
...
조회 요청에는 `AbortController`를 사용했다. 컴포넌트가 사라진 뒤 완료된 요청이 상태를 변경하지 않도록 하기 위해서다.
...
이번 상황에서 오류 원인은 테이블 생성 누락과 일치했다. migration 실행이 성공했고 이후 같은 publishable key 조회에서 오류가 사라졌기 때문이다.
...
`INSERT`가 성공했으므로 `anon` 역할의 테이블 권한과 RLS 정책이 쓰기를 허용한다는 사실을 확인했다. 같은 값을 다시 조회한 결과는 0건이어서 검증용 댓글은 저장소에 남지 않았다.

코드에서는 정상 조회·작성뿐 아니라 존재하지 않는 글, 잘못된 입력, 다른 origin의 쓰기 요청, 저장소 오류 응답을 각각 테스트했다.
...
테스트 통과는 확인한 동작 경로가 기대대로 실행됐다는 근거다. 스팸이나 대량 요청에 대한 안전성을 증명하지는 않는다.
...
트래픽과 스팸이 실제 문제로 확인되면 rate limiting과 moderation을 먼저 추가할 생각이다. 댓글 수가 100개에 가까워지면 cursor pagination도 필요하다.

## 8. 이번 작업에서 배운 점

처음에는 댓글 입력창과 목록을 만드는 일이 작업의 중심이라고 생각했다. 실제로 더 많은 판단이 필요했던 부분은 API, 데이터베이스 권한, 검증 책임을 나누는 일이었다.

> 서버 API와 RLS는 서로를 대신하지 않는다.

API Route는 입력 검증과 일관된 HTTP 응답을 제공한다. 그러나 데이터베이스 권한을 대신하지 않는다. 반대로 RLS만으로는 사용자에게 이해하기 쉬운 오류와 UI 상태를 만들기 어렵다. 두 경계를 분리했을 때 댓글 기능은 화면부터 저장소까지 검증 가능한 흐름이 되었다.
